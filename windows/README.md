...
rollback generation and remove all legacy-recovery, shortcut, startup-task, and
package-local rollback copies unless an active recovery journal or live process
still requires one. Compatible releases load into the current renderer, then an
exact-identity two-phase handoff transfers the binding and owned lock, verifies
the new coordinator heartbeat, and restores the predecessor on failure while
ChatGPT stays open. The v1.5.83-to-v1.5.84 protocol migration performs one
automatic graceful restart. It retains renderer v88 and the ordered Remote
Enabler update, desktop MSIX, and launch gates. Installation and live
multi-device acceptance remain separate checks.

When a verified newer ChatGPT MSIX encounters package resources still in use
(`0x80073D02`) or needs a packaged service that the current user cannot install
(`0x80073D28`), the launcher revalidates and opens the
...
Select **Create Desktop shortcut** and/or **Start at sign-in**, then choose **Apply selected options**. These two choices are optional and unchecked initially. Apply always registers or repairs the **ChatGPT Remote Enabler** Start menu entry and migrates existing legacy aliases to the permanent stable root. Setup does not launch or restart the app. The launcher retains its compatibility filename.
...
controls in both views. Click to prepare that exact verified release and wait
until active work, including internal tasks, finishes. **Cancel** remains
available until installation begins; unknown activity keeps the update queued.
If the protected bridge and publisher files are unchanged, the helper validates
the prepared renderer in the current session, applies it transactionally, and
proves the exact ChatGPT process stayed alive. An update that changes either
protected runtime uses the existing graceful close and relaunch path with the
saved direct/proxy and manual/startup options. Proxy credentials are never
stored in the restart record.
...
Start menu. Setup and update recovery always repair the Start menu entry while
Remote Enabler remains installed; removing its shortcut does not uninstall it.
The launcher always runs the sibling stable and Device Projects bundles, so
...
from their behavior, routes its WebSocket through that local bridge, and maps
the exact bridge URL back to the public target for device-key challenge checks.
Signed challenge data and account authorization remain unchanged. It disables embedded-ASAR integrity
