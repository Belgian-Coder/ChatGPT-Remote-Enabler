...
foreach ($relative in @('windows\CodexRemoteMobileProject\inject.js', 'macos\inject.js')) {
    ...
    foreach ($contract in @('const PROBE_TIMEOUT_MS = 10000;', 'const ENABLE_TARGET_WAIT_MS = 30000;', 'registration.version = report.version', 'connectRendererTargetWithRetry', 'rendererTargetWaitMilliseconds')) {
...
& (Join-Path $root 'tools\Test-UpdateSessionTaskHostTemp.ps1')
if ($LASTEXITCODE -ne 0) { throw 'Windows update-session task-host temp self-test failed.' }
...
[pscustomobject]@{
    ...
    UpdateSessionTaskHostTempSelfTest = $true
