...
const CONTROLLER_PATCHES = [
  ...
  [
    "$Ce=class extends n.an{constructor(e){let t=ZD(e.desktopApiOptions),i=e.globalState,a=e.deviceKeyClient;super({envId:e.hostConfig.env_id,connectionGroup:e.appServerClient,connectionKey:t,websocketUrl:n.on(r.Z(e.desktopApiOptions,`/codex/remote/control/client`)),getAuthHeaders:({headers:t}={})=>$D({appServerClient:e.appServerClient,desktopApiOptions:e.desktopApiOptions,headers:t}),enrollClient:({headers:n})=>eO({appServerClient:e.appServerClient,deviceKeyClient:a,desktopApiOptions:e.desktopApiOptions,enrollmentKey:t,globalState:i,headers:n,onEnrollmentAuthorizationRequired:e.onEnrollmentAuthorizationRequired,requestRemoteControlEnrollmentStepUpToken:e.requestRemoteControlEnrollmentStepUpToken}),authorizeDeviceKeyChallenge:e=>MO({challenge:e,deviceKeyClient:a,enrollmentKey:t,globalState:i})})}}",
    "$Ce=class extends n.an{constructor(e){let t=e.desktopApiOptions,i=e.globalState,a=e.deviceKeyClient,o=e.appServerClient,s=ZD(t);super({envId:e.hostConfig.env_id,connectionGroup:o,connectionKey:s,websocketUrl:process.env.CHATGPT_REMOTE_WS_URL??n.on(r.Z(t,`/codex/remote/control/client`)),getAuthHeaders:({headers:e}={})=>$D({appServerClient:o,desktopApiOptions:t,headers:e}),enrollClient:({headers:n})=>eO({appServerClient:o,deviceKeyClient:a,desktopApiOptions:t,enrollmentKey:s,globalState:i,headers:n,onEnrollmentAuthorizationRequired:e.onEnrollmentAuthorizationRequired,requestRemoteControlEnrollmentStepUpToken:e.requestRemoteControlEnrollmentStepUpToken}),authorizeDeviceKeyChallenge:e=>MO({challenge:e,deviceKeyClient:a,enrollmentKey:s,globalState:i})})}}",
  ],
...
const CHALLENGE_TARGET_PATCHES = [
  ...
  ["E$", "function E$(e,t){let n=new URL(t),r=n.protocol===`wss:`?`https:`:n.protocol===`ws:`?`http:`:null;return r!=null&&e.targetOrigin===`${r}//${n.host}`&&e.targetPath===n.pathname}"],
].map(([name, original]) => ({
  original: Buffer.from(original, "utf8"),
...
function patchDeviceKeyLoader(contents) {
  const audited = findAuditedDeviceKeyLoader(contents);
  const replacement = Buffer.from(
    `return this.addon??=${audited.requireName}(this.resourcesPath+\`/crk.cjs\`)(),this.addon`,
    "utf8",
  );
  patchInPlace(contents, audited.original, replacement, "existing protected device-key loader");
}
