...
(() => {
  ...
  function collectModel() {
    ...
    for (const item of [...tasks, ...nativeProjects]) {
      if (!seenHosts.has(item.hostId)) {
        ...
        hosts.push({ id: item.hostId, name: hostName(item.hostId, names, singleRemoteHostId), available: availability.get(item.hostId) !== false });
      }
    }
  ...
  function nativeFolderIcon(project) {
    const row = nativeProjectRowTemplate(project);
    return [...(row?.querySelectorAll("svg") ?? [])].find((icon) => !icon.closest("button"))?.cloneNode(true) ?? null;
  }
  ...
  function appendGroup(fragment, project) {
    ...
    const folderIcon = project.kind === "recent" ? null : nativeFolderIcon(project);
