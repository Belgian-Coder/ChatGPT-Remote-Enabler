...
function createConnectAgent(proxy, connectTimeoutMs = CONNECT_TIMEOUT_MS) {
  ...
  agent.createConnection = (options, callback) => {
    ...
    deadlineTimer = setTimeout(
      () => reject("Corporate proxy tunnel timed out."),
      connectTimeoutMs,
    );
    deadlineTimer.unref?.();
...
function startBridge(argv = process.argv.slice(2)) {
  ...
  upstream.on("upgrade", (upstreamResponse, connectedSocket, upstreamHead) => {
    tunneledSocket = connectedSocket;
    writeUpgradeResponse(socket, upstreamResponse);
    if (head.length) connectedSocket.write(head);
    if (upstreamHead.length) socket.write(upstreamHead);
    connectedSocket.once("error", () => socket.destroy());
  ...
  parentWatch.unref();
  return { agent, parentWatch, server };
}
