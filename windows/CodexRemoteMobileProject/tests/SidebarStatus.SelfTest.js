...
assert.equal(sidebar.projectStatusIndicator({ ...project, hostId: "remote-control:offline", hostAvailabilityKnown: true, hostAvailable: false, inventoryFresh: false }, false), null, "a stale empty project on a known-offline remote must clear a retained native loading spinner");
assert.ok(sidebar.projectStatusIndicator({ ...project, hostId: "remote-control:online", hostAvailabilityKnown: true, hostAvailable: true, inventoryFresh: false }, false).classList.contains("crmp-project-status-loading"), "a known-online remote must retain genuine native loading state while helper inventory is stale");
assert.ok(sidebar.projectStatusIndicator({ ...project, hostId: "remote-control:unknown", hostAvailabilityKnown: false, hostAvailable: false, inventoryFresh: false }, false).classList.contains("crmp-project-status-loading"), "unknown availability must not be treated as proof that a remote is offline");
