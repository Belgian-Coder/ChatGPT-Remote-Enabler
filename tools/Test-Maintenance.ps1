...
db.close();
'@
    & $node --no-warnings -e $createFixture $testRoot
...
const { DatabaseSync } = require("node:sqlite");
const path = require("node:path");
const db = new DatabaseSync(path.join(process.argv[1], "logs_2.sqlite"), { readOnly: true });
console.log(JSON.stringify(db.prepare("SELECT COUNT(*) AS rows, COALESCE(SUM(estimated_bytes), 0) AS estimated FROM logs").get()));
db.close();
'@
    $remaining = & $node --no-warnings -e $readResult $testRoot | ConvertFrom-Json
