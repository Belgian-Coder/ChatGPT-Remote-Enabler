...
(async () => {
  ...
  const aliasSent = JSON.parse(t.peerTransferText({ ...payload, deviceAliases: { schemaVersion: 1, selfAlias, records: { [host]: recipientAlias, [other]: otherAlias } } }, host));
  assert.deepEqual(aliasSent.deviceAliases.selfAlias, selfAlias);
  assert.deepEqual(aliasSent.deviceAliases.recipientAlias, recipientAlias, "the destination's record must use the authenticated recipient slot, including tombstones");
  assert.equal(aliasSent.deviceAliases.records[host], undefined, "a recipient alias must not be echoed under an untrusted local identity");
  assert.deepEqual(aliasSent.deviceAliases.records[other], otherAlias);
  const forwardedAliases = t.serializePeerInventory(t.parseInventoryPayload(aliasSent)).deviceAliases;
  ...
  t.state.threadInventories.set(host, { error: null, fetchedAt: now, threads: [], truncated: false });
  t.state.localRegisteredProjectsFetchedAt = now - 16000;
  t.state.localRegisteredProjectsPending = true;
  t.state.remoteProjectInventories.set(host, { error: "older helper publisher unavailable", pending: true });
