...
Download a platform ZIP from [v1.5.47 downloads](https://github.com/Belgian-Coder/ChatGPT-Remote-Enabler/releases/tag/v1.5.47), then follow its step-by-step guide:
...
Neither setup requires administrator permissions. A supported desktop app/account and Node.js 22+ are prerequisites; organization policies can still block execution. **v1.5.47 is a normal release. See the validation scope below; source tests and installed-app acceptance are separate.**
...
The interface polish and per-user setup assistants are included in v1.5.47. See [the prioritized feature backlog](UX-ROADMAP.md) for ideas intentionally outside this release.
