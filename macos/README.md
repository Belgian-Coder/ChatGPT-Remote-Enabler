...
Release v1.5.102 moves recognized version-named installations to the fixed
...
repeating the prelaunch check or startup delay. Compatible live updates use an
exact-identity two-phase binding and lock transfer, heartbeat-verify the newly
installed coordinator, and restore the predecessor automatically on failure.
ChatGPT remains open without duplicating the publisher heartbeat. It also ships renderer v88 with
...
Renderer v92 keeps authoritative projectless tasks in Recent chats even when
their working directory matches a registered project, and removes stale
activity indicators from empty devices known to be offline. Its protocol 54
membership marker survives older relays while preserving the path fallback for
project tasks published without a native project ID.

The Dock shortcut now opens a native AppKit progress window immediately and
reports update recovery/check, maintenance, launch, renderer readiness, and
completion. It observes only a private per-user state file, so it does not use
Automation or Apple Events. During an automatic restart, the updater first
requests the exact renderer's native `quit-app` command and can send one
exact-PID POSIX `SIGTERM` only after revalidating the PID, start token,
executable, bundle identity, and user. Changed or ambiguous identities fail
closed without force-killing anything.
...
v1.5.102 is a normal release. Publication is not installation. An older update
...
1. Download **ChatGPT-Remote-Enabler-macOS-arm64-v1.5.102.zip** from [v1.5.102 downloads](https://github.com/Belgian-Coder/ChatGPT-Remote-Enabler/releases/tag/v1.5.102).
...
The v1.5.54 renderer preserves the original folder states, native-backed drag
...
/bin/zsh ./Update-ChatGPTRemote.sh update --proxy
```

Updates now use Git by default, even for extracted installations. Install Git and ensure its HTTPS access to the repository works. The updater lists stable tags, shallow-fetches the pinned commit, and builds/verifies a local package without downloading GitHub ZIPs or calling the GitHub API. In `--proxy` mode, the launcher resolves one fixed credential-free HTTP or HTTPS proxy from its protected per-user configuration, environment, or fixed macOS system-proxy settings and passes it to ChatGPT, Git, and the updater. Loopback debugger traffic remains direct; PAC scripts and authenticated proxies are rejected. A clean source checkout on `main` fast-forwards to that verified tag; dirty work, other branches, SSH origins, or unexpected origins are preserved and refused.
